...
`make login` устанавливает `.app`, регистрирует его как login item через
`SMAppService` (из установленного бинарника, чтобы автозапуск указывал на
`/Applications/CursorUsageTray.app`, а не на debug-сборку) и запускает
приложение. Приложение — агент (`LSUIElement`), запускается без окна и без
терминала, только иконкой в меню-баре.
