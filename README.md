...
(механизм `SMAppService`, встроенный способ macOS).

### Автозапуск через LaunchAgent

Альтернатива меню — переносимый per-user LaunchAgent:

```bash
make login     # включить автозапуск при входе
make unlogin   # выключить
```

`make login` генерирует `~/Library/LaunchAgents/<bundle-id>.plist` из
идентификатора и пути установленного `.app` (`scripts/loginitem.sh`) —
ничего машинно-специфичного не хранится в репозитории, форк с другим
именем/идентификатором работает без правок.
...
Снимает автозапуск и удаляет `/Applications/CursorUsageTray.app`.
